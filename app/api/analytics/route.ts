 ...
 *
 * Returns:
 * - trend_data: reports aggregated by day (last 14 days)
 * - urgency_rankings: clusters ranked by urgency score
 * - anomalies: clusters flagged as abnormal spikes
 * - category_distribution: total breakdown by category
 */
...
// Haversine approximation: degrees to meters at Payatas latitude (~14.7°N)
const LAT_TO_M = 111_320;
const LNG_TO_M = 111_320 * Math.cos((14.7 * Math.PI) / 180);
...
export async function POST() {
  ...
  for (let i = trendDays - 1; i >= 0; i--) {
    ...
    const dayReports = mockReports.filter(
  ...
  // --- 2. DBSCAN clustering for urgency + anomaly ---
  ...
  const clusterIndices = dbscan.run(points, 50, 3); // ε=50m, minPts=3
