...
export default function ReportForm() {
    ...
    return (<Card className={`w-full h-full max-h-full flex flex-col overflow-hidden backdrop-blur-xl shadow-2xl rounded-2xl ${isDark
      ...
      <CardHeader className="px-4 pt-4 pb-2 md:px-5 md:pt-5 md:pb-3 space-y-1.5">
        <div className="flex items-center justify-between">
          ...
          <Badge variant={online ? "default" : "destructive"} className="gap-1 text-[11px] px-2.5 py-0.5">
        ...
        {submitted ? (<div className="flex flex-col items-center gap-3 py-10 text-center">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 sm:grid-cols-4 lg:grid-cols-7 gap-1.5 md:gap-2">
                ...
                return (<button key={cat.key} type="button" onClick={() => setCategory(cat.key)} className={`flex flex-col items-center gap-1.5 py-2.5 md:py-3 px-1 rounded-xl text-center transition-all active:scale-95 ${isSelected
                      ...
                      <span className={`text-[10px] font-medium leading-none ${isSelected
            ...
            <div className="space-y-2">
              ...
              <Textarea id="description" placeholder={t.reportDescriptionPlaceholder} value={description} onChange={(e) => setDescription(e.target.value)} rows={1} className="resize-none text-sm"/>
            </div>
            ...
            <div className="space-y-2">
              ...
              {photoPreview ? (<div className="relative w-full h-32 rounded-xl overflow-hidden">
                  ...
                  <button type="button" onClick={() => setPhotoPreview(null)} className="absolute top-2 right-2 w-7 h-7 flex items-center justify-center rounded-full bg-black/60 text-white hover:bg-black/80">
                    <X className="h-4 w-4"/>
                  </button>
                  <button type="button" onClick={() => fileInputRef.current?.click()} className="absolute bottom-2 right-2 px-2.5 py-1 rounded-lg bg-black/60 text-white text-xs hover:bg-black/80">
                    {t.reportChangePhoto}
                  </button>
                </div>) : (<Button type="button" variant="outline" className={`w-full h-10 gap-2 rounded-xl text-sm ${isDark ? "border-white/10 text-white/70 hover:bg-white/[0.06]" : "border-black/10 text-gray-600 hover:bg-black/[0.03]"}`} onClick={() => fileInputRef.current?.click()}>
            ...
            <div className="space-y-2">
              ...
              <div className="flex gap-1.5">
                ...
                return (<button key={s.value} type="button" onClick={() => setSeverity(s.value)} className={`flex-1 flex flex-col items-center gap-1 py-2.5 rounded-xl transition-all ${isSelected
            ...
            <div className="space-y-2">
              ...
              {latitude !== null && longitude !== null ? (<button type="button" onClick={detectGPS} className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-all ${isDark ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-emerald-50 text-emerald-700 border border-emerald-200"}`}>
                ...
                </button>) : (<Button type="button" variant="outline" className={`w-full h-10 gap-2 rounded-xl text-sm ${isDark ? "border-white/10 text-white/70 hover:bg-white/[0.06]" : "border-black/10 text-gray-600 hover:bg-black/[0.03]"}`} onClick={detectGPS} disabled={gpsLoading}>
