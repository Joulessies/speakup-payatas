...
import { ShieldCheck, FileWarning, Map, Search, TrendingUp, Brain, QrCode, LogOut, UserRound, Menu, X, } from "lucide-react";
export default function NavBar() {
    ...
    return (<>
      ...
      <nav className={`hidden md:flex items-center justify-between gap-3 px-4 lg:px-6 h-14 border-b z-50 backdrop-blur-xl shadow-sm shrink-0 ${isDark
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2 lg:gap-5">
          <Link href="/" className="inline-flex shrink-0 items-center gap-2 leading-none">
            ...
            <span className={`hidden lg:inline text-base font-semibold tracking-tight leading-none ${isDark ? "text-white" : "text-gray-900"}`}>
              SpeakUp
            </span>
          </Link>
          ...
          <div className="flex min-w-0 items-center gap-0.5">
            {visibleMainNav.map((item) => (<Link key={item.href} href={item.href} className={navLinkClass(item.href)}>
        ...
        <div className="flex shrink-0 items-center gap-1">
          <div className="hidden lg:flex items-center gap-0.5">
            {UTIL_NAV.map((item) => (<Link key={item.href} href={item.href} className={navLinkClass(item.href)}>
                <item.icon className="h-4 w-4 shrink-0"/>
          ...
          {session && (<div className={`inline-flex items-center gap-1.5 pl-2 pr-1.5 py-1 rounded-lg border ${isDark ? "bg-white/[0.04] border-white/[0.08]" : "bg-black/[0.02] border-black/[0.06]"}`}>
              ...
              <span className={`max-w-[120px] truncate text-[11px] ${isDark ? "text-white/65" : "text-gray-600"}`}>
                {session.username}
              </span>
              <span className={`text-[10px] px-1.5 py-0.5 rounded ${isDark ? "bg-white/[0.08] text-white/65" : "bg-black/[0.06] text-gray-600"}`}>
                {session.role}
              </span>
              <button type="button" onClick={handleLogout} className={authActionClass}>
                <LogOut className="h-3.5 w-3.5"/>
                Logout
              </button>
            </div>)}
          {!session && (<button type="button" onClick={handleLogin} className={`text-xs px-3 py-1.5 rounded-lg border font-medium transition-colors ${isDark
      ...
      <nav className={`md:hidden fixed bottom-0 left-0 right-0 z-50 border-t backdrop-blur-xl ${isDark
            ? "bg-black/80 border-white/[0.08]"
            : "bg-white/80 border-black/[0.08]"}`}>
        {mobileMoreOpen && (<div className={`mx-3 mb-2 mt-2 rounded-xl border p-2 ${isDark ? "bg-black/90 border-white/[0.08]" : "bg-white/95 border-black/[0.08]"}`}>
            <div className="grid grid-cols-2 gap-2">
              {UTIL_NAV.map((item) => {
                const active = isUtilActive(item.href);
                return (<Link key={item.href} href={item.href} onClick={() => setMobileMoreOpen(false)} className={`inline-flex items-center gap-2 rounded-lg px-3 py-2 text-xs font-medium transition-colors ${active
                        ? isDark
                            ? "bg-white/12 text-white"
                            : "bg-gray-100 text-gray-900"
                        : isDark
        ...
        <div className={`flex items-center justify-between px-3 py-1.5 border-b ${isDark ? "border-white/[0.08]" : "border-black/[0.08]"}`}>
          ...
          {session ? (<button type="button" onClick={handleLogout} className={authActionClass}>
              <LogOut className="h-3.5 w-3.5"/>
              Logout
            </button>) : (<button type="button" onClick={handleLogin} className={`text-[11px] px-2 py-1 rounded-md transition-colors ${isDark
        ...
        <div className="flex items-stretch">
          {visibleMainNav.map((item) => {
            ...
            return (<Link key={item.href} href={item.href} className={`flex-1 flex flex-col items-center justify-center gap-0.5 pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] transition-colors active:scale-95 ${isActive
                    ? isDark
                        ? "text-indigo-400"
                        : "text-indigo-600"
                    : isDark
          ...
          <button type="button" onClick={() => setMobileMoreOpen((prev) => !prev)} className={`flex-1 flex flex-col items-center justify-center gap-0.5 pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))] transition-colors ${mobileMoreOpen
            ? isDark
                ? "text-indigo-400"
                : "text-indigo-600"
            : isDark
